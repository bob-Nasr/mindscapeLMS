...
source_cursor.execute('SELECT idEdType, EductaionType_name, location_idlocation, institution_idInstitution FROM education_type')
...
for idEdType, EductaionType_name, location_idlocation, institution_idInstitution in education_types:
    # Check if the education type name has already been inserted into Moodle
    if EductaionType_name not in inserted_names:
        # Check if the education type already exists in Moodle
        target_cursor.execute('SELECT education_typeid FROM mdl_education_type WHERE education_typename = %s', (EductaionType_name,))
        ...
        if not existing_education_type:  # Education type does not exist in Moodle, insert it
            ...
            inserted_names.add(EductaionType_name)
        else:
            logging.debug(f"Education type '{EductaionType_name}' already exists in Moodle. Skipping insertion.")
    else:
        logging.debug(f"Education type '{EductaionType_name}' has already been inserted into Moodle. Skipping insertion.")
